...
export const ViewportToolbar: React.FC<ViewportToolbarProps> = ({
  ...
  return (
    <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-slate-900/90 backdrop-blur-md border border-slate-700/80 rounded-xl px-4 py-2 flex items-center space-x-4 shadow-2xl z-10 select-none">
      ...
      {/* 3. Botão Exclusivo Rotacionar Palmilha 180° no Eixo Y */}
      {onToggleRotateInsoleY && (
        <button
          onClick={onToggleRotateInsoleY}
          className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all shadow-sm active:scale-95 cursor-pointer ${
            isRotatedY
              ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40'
              : 'bg-slate-800 text-slate-300 border border-slate-700 hover:bg-slate-700 hover:text-white'
          }`}
          title="Rotacionar a Palmilha 180° no Eixo Y"
        >
          <RotateCw className={`w-3.5 h-3.5 text-emerald-400 transition-transform ${isRotatedY ? 'rotate-180' : ''}`} />
          <span>{isRotatedY ? 'Invertida (180° Y)' : 'Girar 180° Y'}</span>
        </button>
      )}

      <div className="h-4 w-px bg-slate-700" />

      {/* 4. Modo de Exibição (Sólido / Wireframe / X-Ray) */}
      <div className="flex items-center space-x-1 bg-slate-800/80 p-1 rounded-lg border border-slate-700">
        <button
          ...
          className={`px-2.5 py-1 rounded text-[11px] font-semibold transition-all ${
            renderMode === 'solid' ? 'bg-blue-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
          Sólido
        </button>
        <button
          ...
          className={`px-2.5 py-1 rounded text-[11px] font-semibold transition-all ${
            renderMode === 'wireframe' ? 'bg-blue-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
          Wireframe
        </button>
        <button
          ...
          className={`px-2.5 py-1 rounded text-[11px] font-semibold transition-all ${
            renderMode === 'xray' ? 'bg-blue-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
          Raio-X (Rx)
        </button>
      ...
      {/* 5. Presets de Câmera 3D */}
      <div className="flex items-center space-x-1 bg-slate-800/80 p-1 rounded-lg border border-slate-700">
        <button
          onClick={() => onSelectViewpoint('perspective')}
          className={`px-2.5 py-1 rounded text-[11px] font-semibold transition-all ${
            viewpoint === 'perspective' ? 'bg-cyan-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
          }`}
        ...
        <button
          ...
          className={`px-2.5 py-1 rounded text-[11px] font-semibold transition-all ${
            viewpoint === 'top' ? 'bg-cyan-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
          }`}
        ...
        <button
          ...
          className={`px-2.5 py-1 rounded text-[11px] font-semibold transition-all ${
            viewpoint === 'lateral_medial' ? 'bg-cyan-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
          }`}
          title="Vista Medial"
        >
          Medial
        ...
        <button
          ...
          className={`px-2.5 py-1 rounded text-[11px] font-semibold transition-all ${
            viewpoint === 'lateral_lateral' ? 'bg-cyan-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
          }`}
          title="Vista Lateral"
        >
          Lateral
